...
ret_t add_edge(graph_t *g, vertex_t v_start, vertex_t v_end, double w){
	...
	assert(insert_end_h(pv_node_start->ph_list, v_end, pv_node_end, w) == SUCCESS); 
	assert(insert_end_h(pv_node_end->ph_list, v_start, pv_node_start, w) == SUCCESS); 
...
void print_graph(graph_t *g, const char *msg){
	...
	for(pv_node = g->pv_list->next; pv_node != g->pv_list; pv_node = pv_node->next){
		...
		for(ph_node = pv_node->ph_list->next; ph_node != pv_node->ph_list; ph_node = ph_node->next){
			printf("[%d | %.1lf]<->", ph_node->v, ph_node->w); 
		}
		puts("[END]"); 
	}
}
...
ret_t prims(graph_t* g, vertex_t r)
{
    vnode_t* pv_r = NULL; 
    vnode_t* pv_run = NULL; 
    vnode_t* pv_u = NULL; 
    ...
    v_prio_queue_t* p_priority_queue = NULL; 

    ret_t status; 

    pv_r = search_node_v(g->pv_list, r); 
    if(pv_r == NULL)
        return (G_INVALID_VERTEX); 

    p_priority_queue = v_create_prio_queue();  

    for(pv_run = g->pv_list->next; pv_run != g->pv_list; pv_run = pv_run->next)
    {
        pv_run->key = INFINITY; 
        pv_run->u_pred = NULL; 
        status = v_prio_enqueue(p_priority_queue, pv_run); 
        assert(status == SUCCESS);     
    }
    
    pv_r->key = 0.0; 

    while(v_is_prio_queue_empty(p_priority_queue) != TRUE)
    {
        status = v_prio_dequeue_min(p_priority_queue, &pv_u); 
        assert(status == SUCCESS); 

        for(ph_run = pv_u->ph_list->next; ph_run != pv_u->ph_list; ph_run = ph_run->next)
        {
            if(v_find_prio_queue(p_priority_queue, ph_run->pv_node) == TRUE)
            {
                if(ph_run->w < ph_run->pv_node->key)
                {
                    ph_run->pv_node->u_pred = pv_u; 
                    ph_run->pv_node->key = ph_run->w; 
                }
            }
        }
    }

    status = v_destroy_prio_queue(&p_priority_queue); 
    assert(status == SUCCESS); 
    return (SUCCESS); 
}
...
void print_mst(graph_t* g, vertex_t r)
{
    ...
    pv_r = search_node_v(g->pv_list, r); 
    puts("Vertices in spanning tree"); 
    printf("[START]<->"); 
    for(pv_run = g->pv_list->next; pv_run != g->pv_list; pv_run = pv_run->next)
        printf("[%d]<->", pv_run->v); 
    puts("[END]"); 
    puts("Edges in spanning tree"); 
    printf("[START]<->"); 
    for(pv_run = g->pv_list->next; pv_run != g->pv_list; pv_run = pv_run->next)
        if(pv_run != pv_r)
            printf("[%d-%d]<->", pv_run->v, pv_run->u_pred->v);   
    puts("[END]"); 
}
...
hlist_t *create_list_h(void){
	hnode_t *ph_head_node = get_node_h(-1, NULL, 0.0); 
	ph_head_node->prev = ph_head_node; 
	ph_head_node->next = ph_head_node; 
	return (ph_head_node); 
} 

ret_t insert_end_h(hlist_t *ph_list, vertex_t v_new, vnode_t* pv_node, double w){
	generic_insert_h(ph_list->prev, get_node_h(v_new, pv_node, w), ph_list); 
	return (SUCCESS); 
}
...
hnode_t *get_node_h(vertex_t v_new, vnode_t* pv_node,  double init_w){
	hnode_t *ph_new_node = (hnode_t*)xcalloc(1, sizeof(hnode_t)); 
	ph_new_node->v = v_new; 
    ph_new_node->pv_node = pv_node; 
...
ret_t v_prio_dequeue_min(v_prio_queue_t *p_prio_queue, vnode_t **ppv_node){
	...
	curr_min = pvq_run->pv_node->key;
	pv_curr_min = pvq_run; 

	while(pvq_run != p_prio_queue){
		if(pvq_run->pv_node->key < curr_min){
			curr_min = pvq_run->pv_node->key; 
			pv_curr_min = pvq_run; 
		}
...
Bool v_find_prio_queue(v_prio_queue_t* p_prio_queue, vnode_t* pv_node){
    v_node_t* run = NULL; 

    for(run = p_prio_queue->next; run != p_prio_queue; run = run->next)
        if(run->pv_node == pv_node) 
            return (TRUE); 
    return (FALSE); 
}
